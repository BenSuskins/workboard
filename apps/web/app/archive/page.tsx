  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-lg font-semibold tracking-tight text-ink">Archive</h1>
      ...
      {shelved.length === 0 ? (
        ...
        <ul className="grid gap-2">
          {shelved.map((p) => (
            <li key={p.id} className="flex items-center gap-3 rounded-[10px] border border-hairline bg-surface px-4 py-3">
              <Link href={`/projects/${p.slug}`} className="truncate text-sm font-medium text-ink hover:text-accent hover:underline">
                {p.name}
              </Link>
              <StatusBadge status={p.status} />
              <span className="hidden truncate text-xs text-muted sm:inline">{p.description}</span>
              ...
              <form action={restoreProjectAction} className="shrink-0">
                ...
                <button
                  type="submit"
                  className="rounded-md border border-hairline px-2.5 py-1 text-xs font-medium text-ink-2 transition-colors hover:border-accent hover:text-accent"
                >
                  Reactivate
                </button>
              </form>
