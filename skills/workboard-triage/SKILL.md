...
`save_report`). If unavailable, tell the user to connect it:
...
1. **Blocked**: `status: blocked` — how long? Is the unblock path named anywhere
   in the updates?
2. **Off track / at risk**: `health: red` or `amber`.
3. **Stale**: status `active` but no updates in the window and `lastActivityAt`
   over a week old.
4. **Rotting PRs**: snapshot shows a PR approved-but-unmerged for days, in
   review with no movement, or a long-lived draft.
5. **Overdue / stuck tasks**: `dueDate` in the past, or `in_progress` tasks on
   otherwise-silent projects.
6. **Contradictions**: e.g. health green but the latest update describes a
